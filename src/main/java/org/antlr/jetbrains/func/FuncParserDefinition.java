...
import org.antlr.jetbrains.func.psi.ArgdefSubtree;
import org.antlr.jetbrains.func.psi.BlockSubtree;
import org.antlr.jetbrains.func.psi.CallSubtree;
...
public class FuncParserDefinition implements ParserDefinition {
	public static final IFileElementType FILE =
		new IFileElementType(FuncLanguage.INSTANCE);
	...
	static {
		...
		List<TokenIElementType> tokenIElementTypes =
			PSIElementTypeFactory.getTokenIElementTypes(FuncLanguage.INSTANCE);
		ID = tokenIElementTypes.get(FuncLexer.IDEN);
	}
	...
	public PsiParser createParser(final Project project) {
		final FuncParser parser = new FuncParser(null);
		return new ANTLRParserAdaptor(FuncLanguage.INSTANCE, parser) {
			@Override
			protected ParseTree parse(Parser parser, IElementType root) {
				// start rule depends on root passed in; sometimes we want to create an ID node etc...
				if ( root instanceof IFileElementType ) {
					return ((FuncParser) parser).program();
				}
				// let's hope it's an ID as needed by "rename function"
				return ((FuncParser) parser).expr();
			}
		};
	}
	...
	public PsiFile createFile(FileViewProvider viewProvider) {
		return new FuncPSIFileRoot(viewProvider);
	}
	...
	public PsiElement createElement(ASTNode node) {
		...
		switch ( ruleElType.getRuleIndex() ) {
			case FuncParser.RULE_function_line :
				return new FunctionSubtree(node, elType);
			case FuncParser.RULE_var_definition :
			case FuncParser.RULE_var_modification:
				return new VardefSubtree(node, elType);
			case FuncParser.RULE_block :
				return new BlockSubtree(node);
			case FuncParser.RULE_function_call_line :
