...
public class FuncStructureViewElement implements StructureViewTreeElement, SortableTreeElement {
	protected final PsiElement element;

	public FuncStructureViewElement(PsiElement element) {
	...
	public ItemPresentation getPresentation() {
		return new FuncItemPresentation(element);
	}

	@NotNull
	@Override
	public TreeElement[] getChildren() {
		if ( element instanceof FuncPSIFileRoot) {
			Collection<? extends PsiElement> funcs = XPath.findAll(FuncLanguage.INSTANCE, element, "/script/function/ID");
			List<TreeElement> treeElements = new ArrayList<>(funcs.size());
			for (PsiElement el : funcs) {
				treeElements.add(new FuncStructureViewElement(el));
			}
